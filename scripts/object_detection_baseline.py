...
def split_annotation_map(annotation_map, train, valid):
    ...
    return {image: annotation_map[image] for image in train_ids}, {
...
train_annotation_map, valid_annotation_map = split_annotation_map(
    train_annotation_map, train, valid
)
...
all_image_ids = (
    [image.split("/")[-1].split(".")[0] for image in train]
...
valid_dataset = SpineObjectDetection(
    valid, valid_annotation_map, anomaly_map, image_id_map, train_transform
)
test_dataset = SpineObjectDetection(
    test, test_annotation_map, anomaly_map, image_id_map, test_transform
)
...
valid_loader = data.DataLoader(
    valid_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn, num_workers=8
)
test_loader = data.DataLoader(
    test_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn, num_workers=8
)
