...
def model_pred_on_test(model, model_h5_file, dataset, batch_size):
    ...
    meta_preds = meta_preds.view(len(test_loader) + 1)
    meta_preds = meta_preds[1:].cpu().numpy()
    meta_labels = meta_labels.view(len(test_loader) + 1)
    meta_labels = meta_labels[1:].cpu().numpy()
    test_acc = running_hits / len(test_loader)
    fpr, tpr, _ = metrics.roc_curve(meta_labels, meta_preds, pos_label=1)
